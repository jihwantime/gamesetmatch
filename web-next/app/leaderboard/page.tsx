    ...
    <>
      ...
      {live && (
        <p className="mt-4 text-xs text-slate-600">
          ...
          are shown separately because ranking points are a rolling 52-week total — blending two
...
function RankTable({ rows }: { rows: BoardRow[] }) {
  return (
          ...
          {rows.map((e) => (
            <tr key={e.id} className="border-t border-white/5 hover:bg-card-2">
              ...
              <td className="px-5 py-2">
                <Link href={`/player/${e.id}`} className="text-slate-200 hover:text-white hover:underline">
